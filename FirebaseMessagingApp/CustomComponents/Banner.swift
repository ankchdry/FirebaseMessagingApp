...
open class Banner: UIView {
    ...
    public let titleLabel: UILabel = {
        let label = UILabel()
        label.font = Font.bold(size: 16.0, font: FontName.SharpSans).fetch()
    ...
    public let detailLabel: UILabel = {
        let label = UILabel()
        label.font = Font.bold(size: 14.0, font: FontName.SharpSans).fetch()
        label.numberOfLines = 0
        label.translatesAutoresizingMaskIntoConstraints = false
        return label
    }()
    ...
    public required init(title: String? = nil, subtitle: String? = nil, image: UIImage? = nil, backgroundColor: UIColor = UIColor.black, didTapBlock: (() -> ())? = nil) {
        ...
        titleLabel.addCharactersSpacing(spacing: 0.8, text: title!)
        detailLabel.addCharactersSpacing(spacing: 0.4, text: subtitle!)
    ...
    private func initializeSubviews() {
        ...
        labelView.addConstraints(NSLayoutConstraint.defaultConstraintsWithVisualFormat("V:|-(20)-[titleLabel]-(15)-[detailLabel]-(10)-|", views: views))
    }
